...
class PolarGridCode():
    ...
    def Encode(self, data):
        ...
        for i in range(len(self.binsr) - 1):
            for j in range(len(self.binsa) - 1):
                # Find the indices of the data that lie within 
                # the current bin for the radius and angle coordinates
                r_indices[(self.binsr[i] <= r) & (r < self.binsr[i + 1])] = i
                a_indices[(self.binsa[j] <= a) & (a < self.binsa[j + 1])] = j

        # Encode the data by selecting the corresponding 
        # bin indices from the bin index matrix
    ...
    def Decode(self, encoded):
        ...
        for i in range(len(self.binsr) - 1):
            for j in range(len(self.binsa)-1):
                # Find the indices of the encoded values that correspond 
                # to the current bin for the radius coordinate
